<template>
	<div>
		...
		>
			<span v-if="!selected" class="opacity-60 text-sm font-dropdown leading-[3rem]"><slot name="dropdown-text"></slot></span>
			<span v-else class="text-sm font-dropdown leading-[3rem]">{{selected}}</span>
			<img
				:src="arrowImg"
				alt="dropdown arrow"
		...
		>
			<li v-for="option in dropdownOptions" :key="option" class="rounded p-2 cursor-pointer hover:bg-adyen-grey hover:underline" @click="setSelected(option)">
				<span>{{option}}</span>
			</li>
		</ul>
	</div>
